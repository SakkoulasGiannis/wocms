...
class UnderConstructionTemplateSeeder extends Seeder
{
    public function __construct(protected TemplateTableGenerator $tableGenerator) {}

    public function run(): void
    {
        if (Template::where('slug', 'under-construction')->exists()) {
            $this->command->warn('Under Construction template already exists — skipping.');
            return;
        }
        ...
        $template = Template::create([
            ...
            'description' => 'Villa projects currently under construction — gallery, location and specifications',
            ...
            'is_public' => true,
            'show_in_menu' => true,
            'menu_label' => 'Under Construction',
            'menu_icon' => 'wrench-screwdriver',
            'menu_order' => 26,
            'has_seo' => true,
        ...
        $fields = [
            ['name' => 'name', 'label' => 'Name', 'type' => 'text', 'description' => 'Villa project name', 'validation_rules' => ['required', 'string', 'max:255'], 'is_required' => true, 'is_searchable' => true, 'is_url_identifier' => true, 'show_in_table' => true, 'order' => 0],
            ...
            ['name' => 'gallery', 'label' => 'Image Gallery', 'type' => 'gallery', 'description' => 'Additional photos of the villa', 'validation_rules' => ['nullable', 'array'], 'is_required' => false, 'show_in_table' => false, 'order' => 3],
            ...
            ['name' => 'year_built', 'label' => 'Year Built', 'type' => 'number', 'description' => 'Expected completion year', 'validation_rules' => ['nullable', 'integer', 'min:1900', 'max:2100'], 'is_required' => false, 'is_filterable' => true, 'show_in_table' => true, 'order' => 5],
        ...
        if (! $this->tableGenerator->createTableAndModel($template)) {
            $this->command->error('Failed to create table and model for Under Construction template');
            return;
        }
        ...
        if ($template->has_physical_file) {
            ...
            $this->command->info('✓ Created physical blade files for Under Construction');
        }

        $this->command->info("✓ Under Construction template created — table: {$template->table_name}, model: {$template->model_class}");
    }
}
