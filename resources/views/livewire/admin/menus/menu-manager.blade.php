<div class="px-4 sm:px-0">
    ...
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            {{-- Export the selected menu as a JSON download. --}}
            <button wire:click="exportJson"
                    @disabled(! $selectedMenuId)
                    class="px-4 py-2 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium rounded-lg shadow-sm transition disabled:opacity-50 disabled:cursor-not-allowed"
                    title="Download the selected menu as JSON">
                <i class="fa fa-download mr-2"></i>Export JSON
            </button>
            {{-- Import a menu from a JSON spec (paste body). --}}
            <button wire:click="openImportModal"
                    class="px-4 py-2 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium rounded-lg shadow-sm transition"
                    title="Import a menu from JSON">
                <i class="fa fa-upload mr-2"></i>Import JSON
            </button>
    ...
    {{-- Import-from-JSON modal — minimal Livewire dialog. --}}
    @if ($showImportModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
            <div class="relative w-full max-w-3xl rounded-xl bg-white shadow-2xl">
                <div class="flex items-center justify-between border-b border-gray-200 px-5 py-4">
                    <div>
                        <h2 class="text-lg font-bold text-gray-900">Import menu from JSON</h2>
                        <p class="text-xs text-gray-500">Paste a spec generated by Export JSON (or hand-crafted with the same shape).</p>
                    </div>
                    <button wire:click="closeImportModal" class="text-gray-400 hover:text-gray-700">
                        <i class="fa fa-times"></i>
                    </button>
                </div>
                <div class="space-y-4 p-5">
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">Mode</label>
                        <div class="flex items-center gap-4">
                            <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                                <input type="radio" wire:model="importMode" value="create" class="text-blue-600">
                                Create new menu (slug auto-numbered if taken)
                            </label>
                            <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                                <input type="radio" wire:model="importMode" value="replace" class="text-blue-600">
                                Replace items of menu with matching slug
                            </label>
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">JSON</label>
                        <textarea wire:model.defer="importJson"
                                  rows="14"
                                  spellcheck="false"
                                  placeholder='{ "version": 1, "menu": { "name": "Main", "slug": "main" }, "items": [ ... ] }'
                                  class="w-full rounded-lg border-gray-300 font-mono text-xs shadow-sm"></textarea>
                        @error('importJson') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                    </div>

                    @if ($importResult)
                        @if (! empty($importResult['ok']))
                            <div class="rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-800">
                                ✅ Imported menu <strong>{{ $importResult['slug'] }}</strong> —
                                {{ $importResult['items_created'] }} items.
                ...
                <div class="flex items-center justify-end gap-2 border-t border-gray-200 px-5 py-3">
                    <button wire:click="closeImportModal" class="px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-lg">Close</button>
                    <button wire:click="runImport" class="px-4 py-2 text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white rounded-lg">Import</button>
                </div>
            </div>
        </div>
    @endif
