...
@php
    ...
    $detailUrl = route('rental-properties.show', $property->slug);
@endphp
<div class="homelengo-box list-style-1 list-style-2 line">
    ...
    <div class="archive-bottom">
        <div class="content-top">
            <h6 class="text-capitalize"><a href="{{ $detailUrl }}" class="link text-line-clamp-1">{{ $property->title }}</a></h6>
            <ul class="meta-list">
                @if($property->bedrooms)
                    <li class="item"><i class="icon icon-bed"></i><span class="text-variant-1">Beds:</span><span class="fw-6">{{ $property->bedrooms }}</span></li>
                @endif
                @if($property->bathrooms)
                    <li class="item"><i class="icon icon-bath"></i><span class="text-variant-1">Baths:</span><span class="fw-6">{{ $property->bathrooms }}</span></li>
                @endif
                @if($property->area)
                    <li class="item"><i class="icon icon-sqft"></i><span class="text-variant-1">m²:</span><span class="fw-6">{{ number_format($property->area, 0) }}</span></li>
                @endif
            </ul>
            @if($property->address || $property->city)
                <div class="location">
                    ...
                    <span class="text-line-clamp-1">{{ $property->address ?? '' }}{{ $property->address && $property->city ? ', ' : '' }}{{ $property->city ?? '' }}</span>
                </div>
            @endif
        </div>
