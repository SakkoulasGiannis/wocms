{{-- Entry Gallery — MASONRY variant (CSS columns).
     Auto-pulls images from the current entry's Spatie media collection.
     Pinterest-style flowing layout that preserves each image's aspect ratio. --}}
@php
    ...
    if (! in_array($columns, [2, 3, 4, 5], true)) { $columns = 3; }
    $gapKey     = $content['gap'] ?? 'normal';
    $lightbox   = filter_var($content['lightbox'] ?? true, FILTER_VALIDATE_BOOLEAN);
    $sectionClass = trim((string) ($content['section_class'] ?? '')) ?: 'py-16 lg:py-20 bg-surface';
...
<section class="{{ $sectionClass }}">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        @if(empty($items))
            <div class="rounded-2xl bg-white p-8 text-center text-sm text-variant-1">
        ...
        @else
            <div
                ...
                <div class="{{ $colsClass }} {{ $gapClass }}">
                    @foreach($items as $i => $g)
                        @if($lightbox)
                            <button type="button" @click="show({{ $i }})"
                                    class="mb-3 break-inside-avoid block w-full overflow-hidden rounded-xl bg-slate-100 ring-1 ring-outline transition-all hover:ring-brand/40">
                        ...
                        @else
                            <div class="mb-3 break-inside-avoid overflow-hidden rounded-xl bg-slate-100">
                                <img src="{{ $g['thumb'] }}" alt="{{ $g['name'] }}" loading="lazy" decoding="async" class="w-full h-auto">
                            </div>
                        @endif
                    @endforeach
                </div>
