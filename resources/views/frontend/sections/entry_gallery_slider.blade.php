{{-- Entry Gallery — SLIDER variant.
     Horizontal scroll slider with snap + arrow controls. Click any slide to
     open the same lightbox. Pure CSS + Alpine — no Swiper dependency. --}}
@php
    ...
    $aspectKey  = $content['aspect'] ?? 'landscape';
    $perViewKey = $content['per_view'] ?? 'multi';
    ...
    $aspectClass = match($aspectKey) {
        'square'   => 'aspect-square',
        'portrait' => 'aspect-[3/4]',
        'wide'     => 'aspect-[16/9]',
        default    => 'aspect-[4/3]',
    };
    $itemWidth = match($perViewKey) {
        'one'  => 'basis-full',
        'two'  => 'basis-full sm:basis-1/2',
        'four' => 'basis-2/3 sm:basis-1/3 lg:basis-1/4',
        default => 'basis-4/5 sm:basis-1/2 lg:basis-1/3',
    };
    ...
    if (isset($entry) && is_object($entry) && method_exists($entry, 'getMedia')) {
        try {
            foreach ($entry->getMedia($collection) as $m) {
                $items[] = [
                    'url'   => $m->getFullUrl(),
                    'thumb' => $m->hasGeneratedConversion('preview') ? $m->getFullUrl('preview') : $m->getFullUrl(),
                    'name'  => $m->name ?? '',
                ];
            }
        } catch (\Throwable $e) {}
        ...
        @else
            <div
                ...
                <button type="button" @click="scrollBy(-1)"
                        class="absolute -left-2 sm:left-2 top-1/2 -translate-y-1/2 z-10 flex h-11 w-11 items-center justify-center rounded-full bg-white shadow-lg ring-1 ring-outline text-on-surface hover:bg-brand hover:text-white transition-colors"
                        aria-label="Previous">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" d="M15 19l-7-7 7-7"/></svg>
                </button>
                <button type="button" @click="scrollBy(1)"
                        class="absolute -right-2 sm:right-2 top-1/2 -translate-y-1/2 z-10 flex h-11 w-11 items-center justify-center rounded-full bg-white shadow-lg ring-1 ring-outline text-on-surface hover:bg-brand hover:text-white transition-colors"
                        aria-label="Next">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" d="M9 5l7 7-7 7"/></svg>
                </button>

                {{-- Slider track --}}
                <div x-ref="track"
                     class="flex gap-3 overflow-x-auto snap-x snap-mandatory scroll-smooth no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
                    @foreach($items as $i => $g)
                        @if($lightbox)
                            <button type="button" @click="show({{ $i }})"
                                    class="snap-start shrink-0 {{ $itemWidth }} {{ $aspectClass }} overflow-hidden rounded-xl bg-slate-100 ring-1 ring-outline transition-all hover:ring-brand/40 group">
                                <img src="{{ $g['thumb'] }}" alt="{{ $g['name'] }}" loading="lazy" decoding="async"
                                     class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105">
                            </button>
                        @else
                            <div class="snap-start shrink-0 {{ $itemWidth }} {{ $aspectClass }} overflow-hidden rounded-xl bg-slate-100">
                                <img src="{{ $g['thumb'] }}" alt="{{ $g['name'] }}" loading="lazy" decoding="async" class="h-full w-full object-cover">
                            </div>
                        @endif
                    @endforeach
                </div>
