...
>
    <div class="mx-auto max-w-8xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-20 items-center gap-8">
            ...
            {{-- Desktop Navigation (homelengo: menu directly after logo, left aligned) --}}
                            ...
                            >
                                ...
                                @if($hasChildren)
                                    ...
                                    <div
                                        ...
                                        class="absolute left-0 top-full min-w-[260px] rounded-2xl bg-white py-2 ring-1 ring-outline shadow-soft"
                                        ...
                                        @foreach($item->children as $child)
                                            ...
                                            >
                                                <a
                                                    ...
                                                    target="{{ $child->target }}"
                                                    class="group relative flex items-center justify-between py-3 pl-6 pr-4 text-sm font-semibold text-on-surface transition-all duration-300 ease-out hover:pl-12 hover:text-brand"
                                                >
                                                    ...
                                                    >
                                                        @foreach($child->children as $grandchild)
                                                            <a
                                                                ...
                                                                class="group relative block py-3 pl-6 pr-4 text-sm font-semibold text-on-surface transition-all duration-300 ease-out hover:pl-12 hover:text-brand"
                                                            >
                                                                <span aria-hidden="true" class="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 -translate-x-2 opacity-0 transition-all duration-300 ease-out text-brand group-hover:translate-x-0 group-hover:opacity-100">↘</span>
                                                                {{ $grandchild->title }}
                                                            </a>
            ...
            <button
                ...
                class="ml-auto inline-flex items-center justify-center rounded-md p-2 text-on-surface hover:bg-surface lg:hidden"
    ...
    >
        <div class="flex h-20 items-center justify-between border-b border-outline px-5">
            ...
            <button
                type="button"
                @click="mobileOpen = false"
                class="rounded-md p-2 text-on-surface hover:bg-surface"
        ...
        <nav class="overflow-y-auto px-5 py-4" style="max-height:calc(100vh - 5rem);">
                        ...
                        <li @if($hasChildren) x-data="{ open: false }" @endif>
                            <div class="flex items-center justify-between">
                                <a
                                    href="{{ $item->resolved_url }}"
                                    target="{{ $item->target }}"
                                    class="flex-1 py-2 text-base font-semibold capitalize text-on-surface hover:text-brand"
                                >
                                    {{ $item->title }}
                                </a>
                                @if($hasChildren)
                            ...
                            @if($hasChildren)
                                <ul x-show="open" x-transition x-collapse class="ml-4 border-l border-outline pl-3" style="display:none">
                                    @foreach($item->children as $child)
                                        <li>
                                            <a
                                                href="{{ $child->resolved_url }}"
                                                target="{{ $child->target }}"
                                                class="block py-1.5 text-sm text-variant-1 hover:text-brand"
                                            >
                                                {{ $child->title }}
                                            </a>
                                            @if($child->children->count())
                                                <ul class="ml-3 border-l border-outline pl-3">
                                                    @foreach($child->children as $grandchild)
                                                        <li>
                                                            <a
                                                                href="{{ $grandchild->resolved_url }}"
                                                                target="{{ $grandchild->target }}"
                                                                class="block py-1 text-sm text-variant-1 hover:text-brand"
                                                            >
                                                                {{ $grandchild->title }}
                                                            </a>
                                                        </li>
            ...
            {{-- Mobile Contact Info --}}
            @if($phone || $email)
                <div class="mt-6 space-y-3 border-t border-outline pt-6">
                    @if($phone)
                        <a href="tel:{{ $phone }}" class="flex items-center gap-3 text-sm text-on-surface">
                    ...
                    @endif
                    @if($email)
                        <a href="mailto:{{ $email }}" class="flex items-center gap-3 text-sm text-on-surface">
                            <svg class="h-5 w-5 text-brand" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
