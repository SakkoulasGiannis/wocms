...
@section('content')
    ...
    <section class="relative bg-slate-900 py-20" style="background-image:linear-gradient(rgba(15,23,42,0.7),rgba(15,23,42,0.7)),url(/themes/kretaeiendom/images/page-title/page-title-4.jpg);background-size:cover;background-position:center;">
        <div class="mx-auto max-w-8xl px-4 sm:px-6 lg:px-8">
            ...
            <h1 class="mt-4 text-4xl font-bold text-white md:text-5xl">Contact Us</h1>
        </div>
    </section>
        ...
        <div class="mx-auto max-w-8xl px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 gap-10 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2">
                    <h2 class="text-2xl font-bold text-slate-900">Drop Us A Line</h2>
                    <p class="mt-2 text-slate-600">Feel free to connect with us through our online channels for updates, news, and more.</p>
                    <div class="mt-8">
                        ...
                        @if($facebook || $instagram || $twitter || $linkedin || $youtube)
                            <li>
                                ...
                                <div class="mt-3 flex items-center gap-2">
                                    @if($facebook)
                                        <a href="{{ $facebook }}" target="_blank" rel="noopener" class="flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-600 shadow-sm transition-colors hover:bg-brand hover:text-white" aria-label="Facebook">
                                            <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                                        </a>
                                    @endif
                                    @if($instagram)
                                        <a href="{{ $instagram }}" target="_blank" rel="noopener" class="flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-600 shadow-sm transition-colors hover:bg-brand hover:text-white" aria-label="Instagram">
                                    ...
                                    @endif
                                    @if($twitter)
                                        <a href="{{ $twitter }}" target="_blank" rel="noopener" class="flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-600 shadow-sm transition-colors hover:bg-brand hover:text-white" aria-label="Twitter">
                                            <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                                        </a>
                                    @endif
                                    @if($linkedin)
                                        <a href="{{ $linkedin }}" target="_blank" rel="noopener" class="flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-600 shadow-sm transition-colors hover:bg-brand hover:text-white" aria-label="LinkedIn">
                                            <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                                        </a>
                                    @endif
                                    @if($youtube)
                                        <a href="{{ $youtube }}" target="_blank" rel="noopener" class="flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-600 shadow-sm transition-colors hover:bg-brand hover:text-white" aria-label="YouTube">
    ...
    @endphp
    <section>
        <div id="contact-map" class="h-96 w-full bg-slate-100"></div>
    </section>
@endsection
