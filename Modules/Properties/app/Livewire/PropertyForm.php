...
class PropertyForm extends Component
{
    ...
    protected function rules(): array
    {
        return [
            ...
            'slug' => 'required|string|max:255|unique:properties,slug,'.$this->propertyId,
    ...
    {
        $p = Property::findOrFail($this->propertyId);
        $this->title = $p->title;
        $this->slug = $p->slug;
    ...
    public function removeGalleryImage(int $mediaId): void
    {
        if ($this->propertyId) {
            ...
            $property->getMedia('gallery')->where('id', $mediaId)->first()?->delete();
    ...
    public function save()
    {
        ...
        $property = Property::updateOrCreate(['id' => $this->propertyId], $data);
        $this->propertyId = $property->id;

        if ($this->featuredImageUpload) {
            $property->clearMediaCollection('featured_image');
            $property->addMedia($this->featuredImageUpload->getRealPath())
                ->usingFileName($this->featuredImageUpload->getClientOriginalName())
        ...
        return $this->redirect(route('admin.properties.edit', $property->id), navigate: false);
    }

    public function render()
    {
        return view('properties::livewire.property-form', [
            'propertyTypes' => Property::getPropertyTypes(),
            'statuses' => Property::getStatuses(),
        ])->layout('layouts.admin-clean')->title($this->propertyId ? 'Edit Property' : 'Create Property');
    }
}
